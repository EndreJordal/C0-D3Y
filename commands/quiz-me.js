  ...
  try {
    ...
    if (answer.customId === correctAnswer) {
      await answer.update({
        content:
          correctReplyArray[
            Math.floor(Math.random() * correctReplyArray.length)
          ] + tip,
        components: [],
      });
    } else {
      await answer.update({
        content:
          `${gptQuiz}\n### ${answer.customId} var dessverre feil, ${correctAnswer} var rett. 😥` +
          tip,
        components: [],
      });
    }
    ...
    await interaction.editReply({
      content: "Du har max 1 minutt på å svare, og tiden er ute 😥",
      components: [],
    });
  }
};
